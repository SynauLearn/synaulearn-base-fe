...
const CoursesPage: FC<CoursesPageProps> = ({
  ...
  const filteredCourses = coursesWithProgress.filter((course: ConvexCourse) => {
    ...
    console.log(matchesLanguage, matchesDifficulty, matchesSearch);

    return matchesLanguage && matchesDifficulty && matchesSearch;
  });

  return (
    <section className="relative min-h-screen w-full bg-sapphire-200/10 pt-5 overflow-hidden">
      ...
      <div className="w-full">
        ...
        />
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[32.5rem] h-[32.5rem] bg-sapphire-200/20 rounded-full">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[21.5rem] h-[21.5rem] bg-sapphire-200/30 rounded-full" />
        </div>
      </div>
      ...
      <div className="absolute top-72 flex flex-col bg-gold-100 min-h-dvh w-full py-6 px-5 rounded-t-[2.25rem] gap-6 z-50">
        ...
        <div className="flex flex-col items-start gap-3">
          {filteredCourses.map((course: ConvexCourse, index: number) => (
            <CourseCard
              key={course._id}
              id={index + 1}
              title={course.title}
